...
SHOW tables CREATE TABLE ds_salaries (
    id INT,
    -- bisa ganti dari Unnamed: 0 kalau mau jadi PK
...
DROP Table ds_salaries LOAD DATA INFILE '/var/lib/mysql-files/ds_salaries.csv' INTO TABLE ds_salaries FIELDS TERMINATED BY ',' ENCLOSED BY '"' LINES TERMINATED BY '\n' IGNORE 1 LINES (
    id,
    work_year,
    experience_level,
    employment_type,
...
FROM
    ds_salaries --1 apakah ada data yang null ?
SELECT
    *
FROM
    ds_salaries
WHERE
    ...
    OR company_size IS NULL --2 melihat ada job title apa saja
SELECT
    DISTINCT(job_title)
FROM
    ds_salaries
ORDER BY
    job_title --3 job title apa saja yang berkaitan dengan data analyst
SELECT
    DISTINCT(job_title)
FROM
    ds_salaries
WHERE
    job_title LIKE '%data analyst%' --4 berapa gaji rata rata data analyst ?
SELECT
    DISTINCT(job_title),
    AVG(salary_in_usd)
FROM
...
GROUP BY
    job_title -- 4.1 Berapa rata-rata gaji data analyst berdasarkan experience levelnya?
SELECT
    experience_level,
